...
import {MorphoPairOracle} from "../oracle/MorphoPairOracle.sol";
...
/// @title MorphoPairOracleBeaconSetDeployerConfig
/// @notice Configuration for the `MorphoPairOracleBeaconSetDeployer`
/// constructor.
...
/// deployer). A zero central reverts `MorphoPairOracle.ZeroCentral` in the
/// implementation constructor.
struct MorphoPairOracleBeaconSetDeployerConfig {
    address initialOwner;
    ST0xPriceOracle central;
}

/// @title MorphoPairOracleBeaconSetDeployer
/// @notice Deploys a beacon and the `MorphoPairOracle` proxies that share it.
...
/// pattern.
///
/// The `MorphoPairOracle` implementation takes the central `ST0xPriceOracle`
...
contract MorphoPairOracleBeaconSetDeployer {
    /// @notice Emitted when a new MorphoPairOracle proxy is deployed.
    /// @param caller The direct on-chain caller of `newMorphoPairOracle`.
    ...
    /// The beacon for the MorphoPairOracle implementation contracts.
    IBeacon public immutable I_MORPHO_PAIR_ORACLE_BEACON;
    ...
    constructor(MorphoPairOracleBeaconSetDeployerConfig memory config) {
        if (config.initialOwner == address(0)) {
            revert ZeroBeaconOwner();
        }

        // A zero central reverts `MorphoPairOracle.ZeroCentral` in the
        // implementation constructor — no local guard needed.
        MorphoPairOracle implementation = new MorphoPairOracle(config.central);

        I_CENTRAL = config.central;
        I_MORPHO_PAIR_ORACLE_BEACON = new UpgradeableBeacon(address(implementation), config.initialOwner);
    }

    /// @notice Deploys and initializes a new MorphoPairOracle proxy.
    ...
    /// constructor via the encoded `initialize` call — `MorphoPairOracle` uses
    ...
    function newMorphoPairOracle(address base, address quote) external returns (MorphoPairOracle oracle) {
        bytes32 salt = keccak256(abi.encode(base, quote));
        oracle = MorphoPairOracle(
            address(
                new BeaconProxy{salt: salt}(
                    address(I_MORPHO_PAIR_ORACLE_BEACON), abi.encodeCall(MorphoPairOracle.initialize, (base, quote))
                )
            )
        );
