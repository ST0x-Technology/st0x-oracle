...
/// helper and the `run()` beacon-owner guard, suite dispatch and required-env
/// reads. Exercised as a plain unit test (no broadcast / fork) via a test-only
/// subclass exposing the internal helper.
contract DeployTest is Test {
    DeployExposed internal deploy;

    address internal constant BEACON_OWNER = address(0x6074E12);

    /// @dev Signature of the beacon-set deployer's `Deployment` event. Used to
    /// discriminate WHICH suite `run()` actually dispatched to: the DIA suite
    /// mints no proxies (zero `Deployment` logs).
    ...
    /// @notice One sequential test covering everything `run()` reads off the
    /// PROCESS env (`DEPLOYMENT_KEY`, `BEACON_INITIAL_OWNER`,
    /// `DEPLOYMENT_SUITE`): the beacon-owner guard, the dia-vault-oracle
    /// dispatch, the unknown-suite fall-through, and the required-not-defaulted
    /// env reads.
    ///
    ...
    function testDeployEnvDispatchAndGuards() external {
        ...
        assertTrue(deployer != BEACON_OWNER, "distinct keys");
        ...
        // deploys infra ONLY — no per-vault proxy is minted, so ZERO
        // `Deployment` logs. Each arm runs its deployment INSIDE
        // `vm.startBroadcast(deploymentKey)` / `vm.stopBroadcast()`, which is
        // what makes DEPLOYMENT_KEY — not the script contract — the on-chain
        // sender of every creation, observable here as the deploy key's nonce:
        // the DIA arm broadcasts TWO creations (the implementation and its
        // beacon-set deployer). An unbroadcast arm would leave the deploy key
        // untouched and attribute the deployment to the script.
        vm.setEnv("DEPLOYMENT_SUITE", "dia-vault-oracle");
        ...
        // who was asked for, broadcast from a key nobody chose. The abort names
        // the failing cheatcode and var, so it can never be confused with one
        // of the script's own `require`s.
        vm.setEnv("DEPLOYMENT_SUITE", "dia-vault-oracle");
        ...
        // A completed DIA dispatch after restoring the env proves the aborts
        // above were the env reads, not a wedged process: nothing is minted
        // (zero Deployment logs) but the two creations broadcast cleanly.
        vm.setEnv("DEPLOYMENT_SUITE", "dia-vault-oracle");
        vm.recordLogs();
        deploy.run();
        assertEq(_countDeploymentLogs(), 0, "dia dispatch mints no proxies after the env is restored");
    }
}
